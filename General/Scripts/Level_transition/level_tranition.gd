@tool
@icon( "res://IKONS/level_transition.svg" )
class_name LevelTransition
extends Node2D

enum  SIDE { TOP, DOWN, LEFT, RIGHT }
@onready var area_2d: Area2D = $Area2D

@export_range( 2, 100, 1, "or_freater" ) var size : int = 2 :
	set( value ):
		size = value
		apply_area_settings()
...
func _ready() -> void:
	if Engine.is_editor_hint():
		return
	SceneMenager.new_scene_ready.connect( _on_new_scene_ready )
	SceneMenager.load_scene_finished.connect( _on_load_scene_finished )

	pass
	
func _on_player_entered( _n :Node2D ) -> void:
	SceneMenager.transition_scene( target_level, target_area_name, get_offset( _n ), "left" )
	pass
